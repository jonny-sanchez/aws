...
resource "aws_security_group" "ec2_sg" {
  ...
  name        = "${each.key}-${local.workspace_suffix}"
...
resource "aws_vpc_security_group_ingress_rule" "ingress" {
  ...
  tags = {
    Name = "${each.value.instance_key}-ingress-${each.value.rule.protocol == "-1" ? "all" : each.value.rule.from_port}"
  }
}
...
resource "aws_vpc_security_group_egress_rule" "egress" {
  ...
  tags = {
    Name = "${each.value.instance_key}-egress-${each.value.rule.protocol == "-1" ? "all" : each.value.rule.from_port}"
  }
}
